...
static_networks=['shark_'+str(n) for n in range(6)]
...
for time_series in ['Poisson','Circadian','Bursty']:
    ...
    for data in static_networks:
        ...
        df=pd.read_csv(data+'_edgelist.csv')
        ...
        new_df.to_csv('../Static_networks_converted/'+data+'_'+time_series+'.csv',sep='\t',index=False)
