...
for colony in [1,2,3]:
  ...
  #      working_df=pd.read_excel('Colony_'+str(colony)+'_trophallaxis_12_hours.xlsx')
    #    working_df=pd.read_excel('Colony_2_trophallaxis_data_fixed_half_second_issue.xlsx',sheetname=density+' density')        
        ...
        working_df=working_df.sort_values('synced_start') # change sort_values to sort for some python versions
...
#                ID_dict[ID]=len(ID_list)-1
        ...
        working_df.to_csv('../../Temporal_networks/ant_trophallaxis/Colony_'+str(colony)+'_'+density+'_formatted.txt', sep='\t')
