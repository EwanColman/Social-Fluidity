...
for i,row in df.iterrows():
    x_value[row['Data']]=position[row['Interaction']]
    y_value[row['Data']]=row['phi']
...
for i in range(57):
    ...
    ax.scatter([x],[y],c=colour[species],linewidth=0,s=80,label=lab)
    #plt.plot([x,x],[y+row['SE'],y-row['SE']],color=colour[species],linewidth=1.2,alpha=1)


plt.plot([0,7],[0,0],color='k')
l=2
angle=169
for i in range(0,7):
...
names=['Aggression','Food sharing','Antennation','Space sharing','Face-to-face','Association','Grooming']
z=0.65
for name in names:
   #plt.annotate 
    plt.text(z,-0.02,name,size=fs,horizontalalignment='left',verticalalignment='top',rotation=-25)
    z=z+1
plt.xticks([])
...
plt.xlim([0,8])
plt.ylim([-0.33,2])            
plt.legend(loc=(0.9,0.14),prop={'size':fs-1},scatterpoints=1,ncol=1,columnspacing=0.3,labelspacing=0.23,handletextpad=0.1)
...
ax.spines['top'].set_bounds(0, 7)
ax.spines['right'].set_position(('data', 7))
...
plt.savefig('../Manuscript/Figures/interaction_types.png',bbox_inches='tight',dpi=256)   
